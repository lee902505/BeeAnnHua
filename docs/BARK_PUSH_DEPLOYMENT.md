...
5. 上傳 V0.12.0 網站檔案到 GitHub Pages，使用已連接 Supabase 的帳號，先抽取一次每日籤，再到 Bark App 確認推播；接著測塔羅與星盤。網站本身若可操作但無推播，檢查 Supabase 的 Edge Function Logs、JWT 設定與金鑰設定。
...
- 網頁上傳本身不會部署 Edge Function 或加入機密設定。完成第 2 至 4 步後，V0.12.0 才會恢復推播。
