...
2. 在現有 Supabase 專案的 **SQL Editor** 執行 `supabase/migrations/20260919_002_bark_push_quota.sql`。它建立原子計數與限制：同一雲端身分每 UTC 日最多 6 次、兩次至少相隔 20 秒；整個網站每天最多 50 次。限制可按實際使用量調整。
...
5. 上傳 V0.11.1.4 網站檔案到 GitHub Pages，使用已連接 Supabase 的帳號，先抽取一次每日籤，再到 Bark App 確認推播；接著測塔羅與星盤。網站本身若可操作但無推播，檢查 Supabase 的 Edge Function Logs、JWT 設定與金鑰設定。
...
- 網頁上傳本身不會部署 Edge Function 或加入機密設定。完成第 2 至 4 步後，V0.11.1.4 才會恢復推播。
