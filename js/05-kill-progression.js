...
function auditTrackKill(mob) {
    ...
    let g = Math.floor((mob.exp || 0) * (1 + partyExpBonusPct() / 100) * (1 + (typeof dollFieldVal === 'function' ? dollFieldVal('expBonus') : 0) / 100) * getGlobalExpMultiplier());   // 🤝 v3.7.62 組隊不再拆分經驗；統計記主玩家完整應得值
    if (g > 0) _audit.exp += g;
    _audit.kills++;
}
...
function killMob(idx) {
    ...
    let _expWithDoll = _expEach * (1 + dollFieldVal('expBonus') / 100);
    let _petExpGain = Math.floor(_expWithDoll * getGlobalExpMultiplier());   // ⭐ 特別版：寵物經驗同樣套用全域倍率；玩家滿等不影響養寵
    let _playerExpGain = Math.floor(_expWithDoll * getExpGainMult(player.lv));   // ⭐ 玩家只套一次全域倍率；Lv100 仍不獲得經驗
    ...
    // === 🔥 50級試煉條件掉落 ===
    if (player.cls === 'knight' && player.trialStage === 1 && mob.n === '黑暗妖精將軍' && !player.inv.some(i => i.id === 'item_dantes_letter') && Math.random() < partyDropRate(0.01 * getGlobalDropMultiplier())) { gainItem('item_dantes_letter', 1); logSys('<span class="text-amber-300 font-bold">✦ 你取得了 丹特斯的召書。</span>'); }
    if (player.cls === 'elf' && player.trialStage === 1 && mob.n === '巨大兵蟻' && !player.inv.some(i => i.id === 'item_ancient_book') && Math.random() < partyDropRate(0.01 * getGlobalDropMultiplier())) { gainItem('item_ancient_book', 1); logSys('<span class="text-amber-300 font-bold">✦ 你取得了 古代黑妖之秘笈。</span>'); }
    if (player.cls === 'dark' && player.trialStage === 1 && mob.n === '黑暗棲林者' && !player.inv.some(i => i.id === 'item_chaos_key') && Math.random() < partyDropRate(0.01 * getGlobalDropMultiplier())) { gainItem('item_chaos_key', 1); logSys('<span class="text-amber-300 font-bold">✦ 你取得了 混沌鑰匙。</span>'); }
    if (player.cls === 'royal' && player.trialStage === 1 && mob.n === '小惡魔' && !player.inv.some(i => i.id === 'item_royal_order') && Math.random() < partyDropRate(0.01 * getGlobalDropMultiplier())) { gainItem('item_royal_order', 1); logSys('<span class="text-amber-300 font-bold">✦ 你取得了 調職命令書。</span>'); }   // 👑 王族 50 級試煉（唯一，不受經典掉率影響，與其他職業一致）
    if (player.cls === 'knight' && player.trialStage === 2 && mapState.current === 'elf_grave' && (player.inv || []).reduce((s, i) => s + (i.id === 'item_elf_whisper' ? (i.cnt || 0) : 0), 0) < 10 && Math.random() < partyDropRate(0.01 * getGlobalDropMultiplier())) { gainItem('item_elf_whisper', 1); logSys('<span class="text-amber-300 font-bold">✦ 你拾起了 精靈的私語。</span>'); }   // 🔧 已持有 10 個則不再掉落（上限）⚠️ v3.5.87 上限口徑＝總持有（含鎖定件）：questCountId 排除鎖定件·用它當上限會被「上鎖」繞過而超收
    if (mob.n === '魔族暗殺團') {
    ...
    if (['安塔瑞斯', '法利昂', '巴拉卡斯', '林德拜爾'].includes(mob.n)) {
        if (Math.random() < partyDropRate(0.10 * getGlobalDropMultiplier())) {
