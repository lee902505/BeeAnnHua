(() => {
  ...
  const T = {
    'zh-CN': {
      brand:'星辰日记', home:'返回首页', eyebrow:'V0.10.7.9 · ACCOUNT LOGIN & OTP',
      ...
      sent:'验证邮件已发送。你可以直接点击邮件里的验证按钮，也可以输入邮件中的 6 位验证码。',
      ...
      otpTitle:'方式二 · 输入 6 位验证码', otpDesc:'邮件内同时提供验证码时，可直接在这里输入完成验证。', or:'或', verify:'验证并完成', testing:'处理中…',
      successBind:'邮箱绑定完成。当前 UUID 已升级为正式云端账号，原有资料仍在同一个账号下。', successChange:'邮箱更换完成。原有云端资料与 UUID 保持不变。',
      loginTitle:'登录已有云端账号', loginSupport:'输入之前绑定过的邮箱。我们会发送登录邮件，你可以点邮件按钮或输入 6 位验证码。', loginSend:'发送登录邮件', loginPending:'登录邮箱',
      loginSent:'登录邮件已发送。请选择「打开邮件验证」或直接输入 6 位验证码。', loginMailTitle:'方式一 · 打开登录邮件', loginMailDesc:'点击邮件里的「登录星辰日记」按钮，会切换回原本的正式会员账号。', loginMailCheck:'我已完成邮件登录',
      loginOtpTitle:'方式二 · 输入 6 位登录码', loginOtpDesc:'如果邮件显示 6 位验证码，可直接输入，不必离开当前页面。', loginVerify:'登录并恢复', loginMemberInfo:'当前已经是正式会员。要恢复账号，请在另一台新设备使用这个入口。',
      restoreChoiceTitle:'检测到这台设备有游客资料', restoreChoiceDesc:'你已经登录原本的正式会员账号。请选择如何处理登录前这台设备留下的游客资料。',
    ...
    'zh-TW': {
      brand:'星辰日記', home:'返回首頁', eyebrow:'V0.10.7.9 · ACCOUNT LOGIN & OTP',
      ...
      bindTitle:'綁定信箱保護帳號', changeTitle:'更換綁定信箱', support:'支援 QQ信箱、Foxmail、163、126、Outlook、iCloud、Gmail 等常用信箱。', emailLabel:'信箱地址', emailPlaceholder:'例如 123456@qq.com', send:'傳送驗證郵件', resend:'重新傳送', sent:'驗證郵件已傳送。你可以直接點擊郵件裡的驗證按鈕，也可以輸入郵件中的 6 位驗證碼。', pending:'待確認信箱', pendingHint:'如果沒有收到郵件，請等待至少 60 秒後再重新傳送。', verifyTitle:'選擇一種驗證方式', mailTitle:'方式一 · 前往驗證郵件', mailDesc:'打開「星辰日記」寄來的郵件，點擊確認按鈕。完成後回到這裡即可。', mailCheck:'我已完成郵件驗證', otpTitle:'方式二 · 輸入 6 位驗證碼', otpDesc:'郵件內同時提供驗證碼時，可直接在這裡輸入完成驗證。', or:'或', verify:'驗證並完成', testing:'處理中…', successBind:'信箱綁定完成。目前 UUID 已升級為正式雲端帳號，原有資料仍在同一個帳號下。', successChange:'信箱更換完成。原有雲端資料與 UUID 保持不變。',
      loginTitle:'登入既有雲端帳號', loginSupport:'輸入之前綁定過的信箱。我們會傳送登入郵件，你可以點郵件按鈕或輸入 6 位驗證碼。', loginSend:'傳送登入郵件', loginPending:'登入信箱', loginSent:'登入郵件已傳送。請選擇「打開郵件驗證」或直接輸入 6 位驗證碼。', loginMailTitle:'方式一 · 打開登入郵件', loginMailDesc:'點擊郵件裡的「登入星辰日記」按鈕，會切換回原本的正式會員帳號。', loginMailCheck:'我已完成郵件登入', loginOtpTitle:'方式二 · 輸入 6 位登入碼', loginOtpDesc:'如果郵件顯示 6 位驗證碼，可直接輸入，不必離開目前頁面。', loginVerify:'登入並恢復', loginMemberInfo:'目前已經是正式會員。要恢復帳號，請在另一台新裝置使用這個入口。',
      restoreChoiceTitle:'偵測到這台裝置有遊客資料', restoreChoiceDesc:'你已經登入原本的正式會員帳號。請選擇如何處理登入前這台裝置留下的遊客資料。', useCloud:'使用原帳號雲端資料（推薦）', mergeGuest:'合併這台裝置的遊客資料', restoreWorking:'正在恢復雲端資料…', restoreDone:'帳號恢復完成，已經切換回原本的正式會員資料。', mergeDone:'帳號恢復完成，這台裝置的遊客資料已與原帳號合併。',
  ...
  function createOtpBoxes(containerId){
    ...
    for(let i=0;i<6;i++){
      ...
      input.addEventListener('input',()=>{input.value=input.value.replace(/\D/g,'').slice(-1); if(input.value&&i<5) host.children[i+1].focus();});
      input.addEventListener('keydown',e=>{if(e.key==='Backspace'&&!input.value&&i>0) host.children[i-1].focus();});
      input.addEventListener('paste',e=>{const code=(e.clipboardData?.getData('text')||'').replace(/\D/g,'').slice(0,6);if(code.length){e.preventDefault();[...host.children].forEach((el,j)=>el.value=code[j]||'');host.children[Math.min(code.length,6)-1].focus();}});
  ...
  function applyText(){
    ...
    $('restoreChoiceTitle').textContent=t('restoreChoiceTitle'); $('restoreChoiceDesc').textContent=t('restoreChoiceDesc'); $('restoreCloudBtn').textContent=t('useCloud'); $('restoreMergeBtn').textContent=t('mergeGuest'); $('restoreFortuneLabel').textContent=t('fortune'); $('restoreTarotLabel').textContent=t('tarot'); $('restoreNatalLabel').textContent=t('natal'); $('restoreSynastryLabel').textContent=t('synastry');
  ...
  function setBusy(b){['sendBtn','resendBtn','verifyBtn','syncBtn','emailAction','loginAction','loginSendBtn','loginResendBtn','loginVerifyBtn','bindCheckLinkBtn','loginCheckLinkBtn','restoreCloudBtn','restoreMergeBtn'].forEach(id=>{const el=$(id);if(el)el.disabled=b})}
